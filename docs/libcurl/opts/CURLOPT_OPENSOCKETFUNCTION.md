...
This callback can be used for IP address blocking of transfers by returning
*CURL_SOCKET_BAD* to block the IP. If a different IP is available for the
transfer then libcurl will try again with that IP. You can read the IP address
information by casting *address-\>addr* to `sockaddr_in` if *address-\>family*
is `AF_INET`, or to `sockaddr_in6` if *address-\>family* is `AF_INET6`. Refer
to *examples/block-ip.c*.
